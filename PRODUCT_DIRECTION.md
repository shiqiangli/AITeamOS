...
- 系统组成方式
- AI Employee Role 模型
- Ticket / Docs-first 流转原则
- Local-first 资产策略
- Employee Chat Workbench 原型边界
- UI 和 Kernel 的职责边界
- 近期验证路径
...
AITeamOS 不是通用 Agent 聊天客户端、IDE、模型应用、Ticket/Docs 替代品或传统任务管理系统。

AITeamOS 是：

> **一个由 AI Employee Roles 组装出来的、本地优先的 Agent Team OS。**

它的目标不是让用户在更多表单里配置更多对象，而是让用户能通过对话直接唤起一组具备角色、能力、记忆、工具和交接规则的 AI Employees，围绕真实项目中的 Ticket/Docs 系统、代码仓库、回归系统和本地知识资产，自动推进任务分解、执行、验证、沉淀和汇报。

一句话描述：

> 给 AITeamOS 一个真实任务，它应能让合适的 AI Employees 自动协作，把任务推进到可验证结果，并把过程、证据、记忆和贡献留存在本地。
...
- **AI Employee Role 管理**：为不同 AI Employee 定义职责、性格、能力边界、Skill、工具权限、Memory 范围、runtime 偏好和交接规则。
- **中立资产层**：Memory、Skill、Knowledge、Prompt trace 和执行证据不绑定到某个商业客户端，而是成为团队本地资产。
- **Ticket-first 协作流转**：任务、状态、沟通和交接尽量复用外部 Ticket 系统，不重复开发用户可见 Task 系统。
- **Harness-first 验证闭环**：真实项目中的回归、构建、测试和发布系统是结果真理源，AITeamOS 负责驱动分析链条收敛。
- **Employee-addressable 协作入口**：用户可以和 Clara 对话，也可以直接点名某个 AI Employee，例如让 Alex 推进某个 ticket 并最终汇报。
- **Trace-first 可观察性**：UI 的价值不是替代 Ticket/Docs 系统，而是让用户看清 Agent 团队背后发生了什么。
- **外部能力复用**：能复用现有 Agent IDE、CLI、API、MCP、CI、Ticket/Docs、Memory 框架，就不在 AITeamOS 内重做。
- **团队贡献量化**：记录不同 AI Employee 在问题分析、排除假设、交接、验证和沉淀中的有效贡献。
...
```text
User
  -> Employee Chat Workbench
  -> Clara or addressed AI Employee
  -> Thin Kernel
  -> AI Employee Roles
  -> EmployeeRuntimeAdapter
  -> Mature Agent Executors / Tools / MCP / External Systems
  -> Ticket Flow + Docs + Local Trace + Memory + Skills
  -> Trace-first UI
```

关键组成如下：

- **Employee Chat Workbench**：用户主要入口。用户可以默认与 Clara 对话，也可以直接选择或点名某个 AI Employee，提出目标、约束、runtime 偏好和人工判断。
- **Thin Kernel**：极薄控制层。负责加载配置、管理权限、监听事件、唤醒成员、防重复触发和保存最小 trace。
- **AI Employee Roles**：AITeamOS 的主要功能由 AI Employees 完成，而不是由传统后端业务模块完成。
- **EmployeeRuntimeAdapter**：把 AI Employee 的 profile、Skill、Memory、权限和 trace 边界映射到外部成熟 Agent、Agent CLI、模型 API、MCP 或本地 runtime。
- **Mature Agent Executors / Tools / MCP / External Systems**：Cursor、Claude Code、Deep Agents、OpenAI/Anthropic/DeepSeek provider、Plane、Repo、Harness、Memory、Skill、Runtime 等都作为可复用执行能力暴露给 AI Employees。
- **Ticket Flow**：外部 Ticket 系统是任务与协作现场，Docs 系统是显性项目知识现场。
- **Local Assets**：本地保存 Employee profiles、Skills、Memory 策略、Prompt/Context trace、执行证据和 Memory candidates。
- **Trace-first UI**：展示流转路径、证据、Prompt、Memory、runtime、Harness 结果和贡献。

设计原则：

- 能由 AI Employee 调工具完成的，不优先做成表单。
- 能复用外部系统的，不优先自研。
- 能由外部 Ticket 系统表达的任务状态，不在 AITeamOS 内重复建 Task 系统。
- 能直接对话唤起某个 Employee 的，不强制绕行 Clara。
- AITeamOS 自己只保留让 Agent 团队稳定运行所必需的框架、策略、权限和 trace。

---

## 5. AI Employee Role 模型

AI Employee Role 是 AITeamOS 的核心抽象。

一个 Role 不是固定后端模块，而是一份可配置的 AI Employee profile。它描述这个 Employee 能做什么、不能做什么、如何使用工具、如何访问记忆、如何与其他 Employee 交接，以及在什么条件下升级给人。

用户可以直接与任意 AI Employee 对话。Clara 是系统初始化创建的默认 AI Employee 和团队运营入口，但不是唯一对话对象。直接对话的典型形式包括：

```text
Alex，请你推进 ticket AIT-1234，必要时找 Release/PV 协作，最后汇报结果。
```

被点名的 Employee 应按自己的 profile、Skill、Memory 策略、工具权限和升级规则行动；当任务需要拆分、转交或全局协调时，它可以把控制权交给 Clara 或触发 orchestration capability。

每个 AI Employee Role 应包含：
...
| Employee | 核心职责 |
|---|---|
| **AI Team OS Manager** | 用户入口、团队运营、系统资产管理、需求理解、Ticket 创建或选择、全局协调、最终汇报 |
...
| **AI Trace Reporter** | 根据 Ticket、trace、Memory 和 Harness 结果生成可观察视图 |

Clara 的职责边界必须保持清晰：Clara 是 **AI Team OS Manager / control-plane manager**，不是 Architecture/Tech Lead、RD 或 PV。她是系统默认 Employee，系统初始化时自动创建，并且永远不允许删除。她可以管理 Employee、Skill、Memory、Knowledge access、Capability 和 Ticket 等控制面资产；也可以读取 Knowledge、创建或选择 Ticket、委派给合适 Employee、收集报告、要求 PV 验证，并基于 LLM 对多方材料做一致性检查；但她不应直接读取代码仓库状态、执行 repo edit、运行测试或承担技术细节判断。代码仓库取证、技术方案、实现和验证应由 Architect、RD、PV 等 Employee 完成，并以 report、trace、evidence、Decision candidate 或 Memory candidate 回传给 Clara。
...
- 观察 Ticket 当前状态
- 判断下一步是否需要唤醒某个 Employee
- 判断是否需要等待外部事件
- 判断是否需要触发 Harness
- 判断是否需要变更 assignee、status、link 或创建关联 Ticket
- 判断是否失败、超预算、卡住或需要人工升级

实现上可以有两种形态：

- **原型阶段**：由 AI Team OS Manager 内置 orchestration mode，同一个 Employee 负责用户对话和后台推进。
- **复杂流程阶段**：拆成独立 AI Orchestrator，用于多 Ticket、长链路、定时轮询、失败重试、fan-out / fan-in 和预算控制。

无论哪种形态，用户都不需要理解 Orchestrator。Orchestration 是 AITeamOS 的执行策略，不应变成新的表单入口。

---

## 6. Ticket / Docs-first 流转原则

AITeamOS 不开发用户可见 Task 系统，也不开发 Confluence 替代品。

文档中保留的“Ticket/Docs”可以理解为用户熟悉的概念速记：AITeamOS 真正依赖的是外部系统暴露的 **Ticket capability** 和 **Docs capability**。从 1.0 方向看，默认且唯一主线的外部落点固定为 **Plane**：

- Plane Ticket 承载 Ticket / ticket。
...
- Plane workspace、project、teamspace、employee 和 permission 承载分组和权限边界。

选择 Plane 作为默认 Ticket/Docs 后端后，AITeamOS 暂不实现 Redmine connector，也不同时维护多套 ticket 系统适配。只有当 AITeamOS 未来需要兼容更广泛团队生态时，再引入 Redmine、Ticket/Docs、OpenProject 或其它兼容 connector。
...
- 体验现代，Tickets、Pages、comments、relations、attachments 和 project/workspace 语义更贴近 AI 团队协作。
- 官方 REST API 覆盖项目、Tickets、states、comments、links、activities、Pages、employees 等 AITeamOS 需要的事实账本能力。
- 官方 MCP Server 路线与 AI Employee 工具调用天然相容，后续可直接复用而不是自研协议层。
- Docker self-host 是官方路径，适合由 AITeamOS 作为 managed external service 固定版本、启动、监控和备份。
- AITeamOS 的主操作入口仍是 Chat；Plane 提供现代、可审计的 Ticket/Docs 事实现场，而不是替代 Clara flow。
...
代码仓库不升格为复杂 Project 模型。Plane 已经承载 project、ticket、page、成员和权限；AITeamOS 只需要知道某个 Plane workspace/project 关联哪些代码仓库：
...
- 限制读取文件大小，并只读取文本文件。
- 如果消息包含 Ticket id，Employee 的 repo inspection 可以写回 Ticket report。
...
AI Employee 不需要在 comment 中重复标注自己的 role。当前负责方应由 Ticket assignee 或 AITeamOS 归一化后的 assignee 表达。
...
User
  -> Clara 理解目标
  -> Clara 创建或选择 Ticket
  -> Ticket 写入目标、上下文、验收条件和初始路由信息
  -> Kernel 监听 Ticket 事件或定时查询
  -> 如果 assignee 明确，唤醒对应 Employee
  -> 如果下一步不明确，触发 orchestration capability
  -> Employee 读取 Ticket、Docs、Memory、Skill、Repo、Harness 等上下文
  -> Employee 执行分析或动作，并更新 Ticket
  -> Ticket assignee / status / relation / comment 驱动下一步
  -> Harness 或人工判断提供验证信号
  -> Clara 汇总链条结果
```

AI Employee 可以根据上下文选择：

- 在现有 Ticket comment 中继续分析
- 修改 assignee
- 修改 status
- link 到已有 Ticket
- 创建新 Ticket 或 subtask
- 读取或更新相关 Docs
- 触发 Harness
- 请求人工判断
- 汇总并关闭链条

核心不是“创建更多 ticket”，而是让 Ticket assignee、status、comment、relation、Docs 和外部验证信号驱动问题收敛。
...
Employee profiles、Skills、Memory policy、runtime 配置、provider thread 映射、conversation history 和 trace 应优先保存在本地文件中，例如：
...
Skill 是 AI Employee 可复用的能力资产。初始版本应优先本地文件化，避免过早依赖外部 Skill Registry。

Clara 或被授权 Employee 应能通过对话创建、修改和分配 Skill。例如：
...
UI 可以展示 Skill 列表、Skill 详情、Role 的 Skills 列表，以及某次 Ticket 流转中使用了哪些 Skills，但创建和分配不应以表单为主路径。
...
- **Memories**：从执行、沟通、Ticket resolve、验证和复盘中沉淀的时间性事实与经验。
...
Clara 在拆解和委派前应调用 `search_knowledge`，读取相关 Docs、Decisions 和 approved Memories。需要代码仓库状态时，她应委派给对应 Employee 取证，再把 Employee report 与 Knowledge、Ticket、trace 交给 LLM 做交叉验证。AITeamOS 不应把这种交叉验证硬编码成规则引擎。
...
AITeamOS Memory Policy / Audit UI
  ...
  -> Chat / Ticket / Decision / Project-doc recall
```
...
- **Memory candidate**：由 AITeamOS 从 trace、conversation、Ticket、Docs 和 runtime output 中记录候选，经过 approval 后才写入长期 memory backend。
...
同一个 Memory 后端应优先通过 employee、project、ticket、run、organization 或 metadata 做隔离。只有强安全隔离场景才需要为每个 Employee 运行独立 Memory 实例。

Knowledge UI 的 Memories / Review Queue 职责是观察和治理，不是重做 Memory 后端：

- 展示本次 run 召回了哪些 memory。
- 展示哪些 memory candidates 被生成、批准或拒绝。
- 展示 memory 来源 trace、关联 Ticket、scope、置信度和最后使用时间。
- 控制某个 Employee、项目或 Ticket 可访问哪些 memory scope。
...
P0 中 MCP 先落为 file-backed connector registry，例如 `.aiteamos/mcp_connectors.json`。Registry 记录 connector id、transport、enabled/configured 状态、capabilities、permissions 和 required settings。Clara 和 Employee 只读取 AITeamOS 归一化后的 capability，不直接依赖某个外部 MCP server 的配置形状。Plane 作为默认 Ticket/Docs connector 优先挂到 registry 后面；Redmine、Ticket/Docs、OpenProject 等兼容 connector 暂不实现，除非未来需要更广泛的 ticket 系统兼容。

AITeamOS 不应在产品模型里区分 Ticket/Docs 系统是本地部署还是 Cloud。对 Kernel、Clara 和 Employee 来说，它们都是 connector 暴露的稳定 capability，例如 `knowledge.docs.search`、`knowledge.docs.read`、`knowledge.docs.write`、`tickets.search`、`tickets.create`、`tickets.comment` 和 `tickets.transition`。部署形态只体现在 connector settings 里的 API base URL、auth 和 adapter normalization；UI 不提供 local/cloud 分叉路径。
...
- **Skill** 是 Employee 的工作方法、流程和判断标准，可以声明需要哪些 tools 或 knowledge scopes。
- **Tool** 是确定性可执行动作，例如 `create_ticket`、`search_knowledge`、`inspect_code_repository`。
...
进入 1.0 后，Chat tool planner 应从 Capability Registry 读取可用 tool schema，而不是在多个模块里维护重复工具清单。Employee profile 则引用 permissions、Skill 和 Knowledge scopes，表达“谁能用哪些能力”。
...
Clara 必须有默认 runtime，否则系统无法启动自动化流程。普通 Employee 不应硬绑定某个 API。用户可以在对话中指定本次使用默认 runtime、特定 LLM API、Agent CLI、IDE Agent、本地模型或 CI 能力。

AITeamOS 不应抹掉商业或开源 Agent 自身的长期上下文能力。如果某个 AI Employee 长期绑定同一个外部 agent、provider thread、IDE workspace 或 CLI session，外部 agent 可以继续积累它自己的经验。AITeamOS 在它前面增加的是一层本地可控的上下文闸门：

- 决定本次发送哪些 Employee profile、Skill、Memory、Ticket、Docs 和工具上下文。
- 记录哪些上下文被发送给了哪个 runtime/provider/thread。
- 约束权限、成本、安全和升级规则。
- 把执行过程、证据和 memory candidate 留存在本地。

成熟 Agent IDE、Agent CLI 和 Deep Agent runtime 是 AITeamOS 优先复用的执行层。AITeamOS 不做 repo indexing、diff/patch UI、terminal agent、PR 编辑器、通用 coding agent 或通用 Agent workbench；这些能力应由 EmployeeRuntimeAdapter 委托给外部执行器。AITeamOS 只决定调用哪个执行器、附带哪些上下文、使用哪个 session/thread、允许哪些工具和如何把结果回写到 Ticket、Docs、trace、Memory candidate 或 Skill candidate。

AITeamOS 关注的是：

- 哪个 Employee 在什么上下文下使用了哪个 runtime
- runtime 输入了哪些 prompt、memory 和工具结果
- runtime 输出如何进入 Ticket、Docs、trace、Memory candidate 或 Harness 流程
...
Ticket/Docs 系统是协作现场，但不适合承载所有执行细节。
...
Ticket 负责流转，Docs 负责显性知识，Harness 负责验证，Memory 负责沉淀，Trace UI 负责解释过程。

---

## 9. UI 模型

AITeamOS UI 的主要价值不是替代 Ticket/Docs 系统，也不是做一个更复杂的管理后台。

UI 应服务两个目标：

- **Conversation-first 操作**：用户主要通过 Employee Chat Workbench 提出目标、补充约束、指定 runtime、批准高风险动作和要求汇报。Clara 是默认入口，但用户也可以直接选择或点名任意 AI Employee。
- **Trace-first 观察**：用户能看清 Ticket 背后的 Agent 团队路径、证据、Prompt、Memory、runtime、Harness 结果和贡献。
...
`Tickets` 是 AITeamOS 对 Plane / 本地 Tickets 的 AI 运行视图，而不是另一个项目管理系统：
...
Ticket Flow Trace 可以展示：

- Ticket、subtask、relation 的流转图
...
后续可以增加 Employee View、Memory View、Skill View、Project View、Prompt Path View 和 Contribution View，但这些都应是观察视图，不应变成主要表单操作中心。
...
- 加载 Employee profiles、Skills 和 runtime 配置
- 管理工具权限、secret 和安全边界
- 暴露 Tools / MCP capability
- 监听 Ticket webhook、Ticket 查询或定时触发器
- 唤醒对应 Employee 或 orchestration capability
- 防止同一 Ticket 被重复触发或并发踩踏
- 保存最小 trace 和执行证据
- 执行预算、权限、重试、停止和人工升级策略

Kernel 不应负责：

- 代替 AI Employee 做复杂产品判断
- 固化所有业务流转逻辑
- 重做 Ticket/Docs、CI、Memory DB、IDE、Agent runtime 或通用聊天客户端
- 把能力重新包装成传统 CRUD 模块
...
- Plane 或其它外部 Ticket/Docs 系统的替代品
- 通用 Agent 聊天客户端
- Agent IDE 或 IDE 替代品
...
- 这件事能否由 AI Employee 调用工具完成？
- 是否已有成熟工具、MCP、开源框架或商业系统可复用？
- 是否真的需要新表单、新表或新内部任务系统？
- 它是否增强了 Role、Ticket Flow、Docs、Memory、Skill、Harness、Trace 或本地资产沉淀？
...
### P0：Employee Chat Workbench

用户能通过 Employee Chat Workbench 与 Clara 或任意 AI Employee 对话：

- 选择或点名一个 AI Employee，例如 Alex、PV、RD 或 Clara
- 绑定或识别一个 Ticket key
- 用本地文件加载 Employee profile、Skill、Memory policy 和 runtime 偏好
...
- 每轮请求前由 AITeamOS 组装 Employee profile、Skill、Memory、Ticket、Docs 和权限上下文。
...
在真实 runtime 接通后，Employee Chat Workbench 应接入 assistant-ui 作为前端聊天 primitives/runtime，而不是长期维护自研聊天框架。assistant-ui 负责 composer、message list、运行状态、自动滚动和后续 tool call 展示；AITeamOS 仍负责 employee routing、runtime settings、context gate、trace、provider state 和 file-backed persistence。
...
assistant-ui
  ...
  -> AITeamOS EmployeeRuntimeAdapter
...
- **EmployeeRuntimeAdapter** 负责把 Employee 身份、上下文、权限和 provider/session/thread 映射到外部成熟 Agent 或模型 provider。
- **AITeamOS Kernel** 只负责产品内核：Employee、Skill、Memory、permission、trace、provider state、deterministic local tools 和对成熟 Agent 的委托边界。
...
- `start`：返回 thread、run、target employee、provider thread 和 Ticket keys。
- `delta`：返回增量文本。
...
### P0.3.2：Employee Threads and Short-Term State

每个 Employee 应有稳定的默认 conversation thread，例如 `employee-clara-default`。Thread metadata 由后端 file-backed index 管理，记录 `employee_id`、title、active thread、message count、updated time 和 archived 状态；前端只负责通过 assistant-ui history/thread adapter 加载和切换，不再把 employee/thread 映射作为浏览器本地事实源。

Conversation history、LangGraph checkpoint 和长期 Memory 必须分层：

- **Conversation history**：用户和 Employee 看得见的 transcript，用于 UI 恢复和短期上下文回放。
...
- 加载 Employee profiles
- 加载本地 Skills
- 加载默认 runtime
- 暴露 Ticket、Docs、Memory、Skill、Repo、Harness、Runtime tools
- 监听 Ticket 并唤醒 Employee
- 记录最小 trace
- 管理权限、secret 和重复触发

### P2：Ticket Flow Trace

第一版 UI 只需清楚展示一个 Ticket 链条：
...
- Skill 能关联到 Employee
- Employee 执行时能使用 Skill
- Memory 能按 Employee、项目和 Ticket 上下文召回
...
- Release、PV、RD 等 Employees 能通过 Ticket 流转协作
- Harness 结果能驱动下一步判断
...
AITeamOS 应以“AI Employee 团队是否能在真实项目中自动推进问题收敛”来衡量成功。

关键指标包括：

- Clara 创建或推进 Ticket 的数量
- AI Employees 完成的有效交接次数
...
- Skill 创建数、复用率和关联覆盖率
- Prompt、Memory、runtime、Harness 和 Ticket trace 完整率
- 不同 Employee 的有效贡献占比
...
> 用极薄 Kernel 和本地优先资产层，把一组 AI Employee Roles 组织成可以围绕 Ticket/Docs 和真实工程系统自动协作的 Agent Team OS。

它应该持续遵守三条原则：

- **Agent-first**：凡是能由 AI Employee 调用工具完成的，不优先做成表单。
